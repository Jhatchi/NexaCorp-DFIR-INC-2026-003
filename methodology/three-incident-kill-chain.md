...
This note is the standalone kill chain narrative behind the cross-incident analysis of the [Month 1 Assessment Report](../reports/Month1_Assessment_Report.md). It explains how three incidents reported across three weeks at NexaCorp Industries are assessed as a single Month 1 campaign.
...
- **INC-2026-003:** Using one of those stolen keys (`svc_api`, which a misconfigured `authorized_keys` accepted for the `svc_backup` account), the attacker reached `lge-files-01` over SSH, escalated to root through a `sudo NOPASSWD` rule on python3, and installed two persistence mechanisms (the `sysupdate` backdoor account and a cron-based C2 beacon to 34.251.89.142).
...
4. **What connects the incidents to the same actor?** The strongest link is between INC-2026-002 and INC-2026-003, and it is evidenced: the same `svc_api` SSH key, Tor exit nodes in the 185.220.101.x range, and a privilege-escalation-then-persistence pattern (a SUID misconfiguration on `bru-app-01`, then a `sudo NOPASSWD` rule on `lge-files-01`, each followed by durable persistence). INC-2026-001 is connected by inference, not by a shared fingerprint: it is the foothold where the reused key material was most plausibly harvested. Its captured activity used a different external IP (172.16.50.10) over a vsftpd backdoor, and the same host carried a separate Caldera C2 implant likely operated by a distinct actor. The three are assessed together as one Month 1 campaign per the engagement scope, with the 002-to-003 link evidenced and the 001 link inferential.
...
The behavioural signature is consistent across INC-2026-002 and INC-2026-003, with INC-2026-001 as the originating foothold: valid credentials over guessing, deliberate host-to-host chaining in a logical sequence, and patient steps to remain hidden and durable. This is targeted activity by an actor who understood the environment, not opportunistic background noise.

See [`attack-timeline.md`](attack-timeline.md) for the minute-by-minute INC-2026-003 sequence and [`attck-mapping.md`](attck-mapping.md) for the INC-2026-003 MITRE ATT&CK mapping.
